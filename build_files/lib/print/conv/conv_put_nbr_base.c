...
static void my_put_nbr(my_lli_t nb, char const *base, int *c_tt)
{
    my_lli_t c = nb;
    ...
    if (c >= 1) {
        ...
        my_put_char(base[c % base_length], c_tt);
    }
}
...
void conv_to_bin(va_list list, int *char_total, flag_t *Flag)
{
    ...
    if (Flag[0].is_hashtag)
        my_put_str("0b", char_total);
    if (nb == 0) {
        my_put_char('0', char_total);
        return;
    }
    my_put_nbr_base(nb, bin, char_total, Flag);
}
